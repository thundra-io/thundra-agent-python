...
def test_send_report_to_url(mock_requests, mock_report, monkeypatch):
    ...
    reporter.session.post.assert_called_once_with(post_url, data=json.dumps([mock_report], separators=(',', ':')), headers=headers, timeout=constants.DEFAULT_REPORT_TIMEOUT)
